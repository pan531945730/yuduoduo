<template>
	<ul class="house-list-content scroll-wrap" :style="{'min-height': minHeight + 'px'}">
		<li class="house-item tap-active" v-for="(item, index) in listData" :key="item.HID+item.ParentID"
		    @click="goDetail(item)">
			<div class="box">
...
export default {
    ...
    methods: {
        goDetail(obj) {
            this.$router.push(`details?code=${ obj.HID }&pid=${ obj.ParentID }&rental=${ obj.Rental }`)
	    },
        tstart(e) {
	        let item = e.target
