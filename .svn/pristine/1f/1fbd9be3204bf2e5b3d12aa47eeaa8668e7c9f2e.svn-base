<template>
    <div class="details-body" v-show="bodyShow" :class="{'android': isAndroid}">
        ...
        <div class="appoint">
            <router-link :to="'/appoint?code=' + Hid + '&pid=' + Houseid" class="appoint-btn">在线预约</router-link>
            <a href="tel:400-025-6788" class="call-number">
                <i class="icon-phone"></i>400-025-6788</a>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        init(code, pid, rental) {
            this.Hid = code || getUrlParams('code', this.url)
        ...
        toRecommend(houseid, hid, rental, status) {
            if(hid != this.nowHouseID && status == 1){
                ...
                this.url = setUrlParams('code', hid, this.url)
